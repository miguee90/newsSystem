angular.module('globalmediaApp').controller('busquedaCtrl', function ($scope,$timeout,$http, $stateParams) {
	var INTERVAL = 10000;
	...
	$http.get('/busca/'+$scope.param+'/'+1).then(function(response){
		$scope.investigaciones=response.data.objetos;
	...
	$scope.cargaMas=function(){
		pagina++;
		$http.get('busca/'+$scope.param+'/'+pagina).then(function(n){
