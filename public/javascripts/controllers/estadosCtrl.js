angular.module('globalmediaApp').controller('estadosCtrl', function ($scope, $state, $timeout,$http, $location, $anchorScroll) {
	var INTERVAL = 10000;
	...
	$http.get('/secciones/'+'Estados').then(function(portada){
	...
	/*GET PARA TOP NEWS*/
	$http.get('/popularSeccion/'+'Estados').then(function(tops){
	...
	$scope.cargaMas=function(){
		pagina++;
		$http.get('cargaMas/'+pagina+'/'+7).then(function(n){
