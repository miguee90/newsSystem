angular.module('globalmediaApp').controller('negociosCtrl', function ($scope, $state, $timeout,$http, $location, $anchorScroll) {
	var INTERVAL = 10000;
	...
	$http.get('/secciones/'+'Negocios').then(function(portada){
	...
	/*GET PARA TOP NEWS*/
	$http.get('/popularSeccion/'+'Negocios').then(function(tops){
	...
	$scope.cargaMas=function(){
		pagina++;
		$http.get('cargaMas/'+pagina+'/'+6).then(function(n){
