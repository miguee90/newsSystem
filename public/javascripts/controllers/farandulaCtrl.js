angular.module('globalmediaApp').controller('farandulaCtrl', function ($scope, $state, $timeout,$http, $location, $anchorScroll) {
	var INTERVAL = 10000;
	...
	$http.get('/secciones/'+'Farandula').then(function(portada){
	...
	/*GET PARA TOP NEWS*/
	$http.get('/popularSeccion/'+'Farandula').then(function(tops){
	...
	$scope.cargaMas=function(){
		pagina++;
		$http.get('cargaMas/'+pagina+'/'+10).then(function(n){
	...
	$scope.anoticia=function(alias,id){
		$state.go('noticiaId',{id:id,alias:alias});
	};	
});