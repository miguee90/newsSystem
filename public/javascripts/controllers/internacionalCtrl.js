angular.module('globalmediaApp').controller('internacionalCtrl', function ($scope,$state ,$timeout,$http, $location, $anchorScroll) {
	var INTERVAL = 10000;
	...
	$http.get('/secciones/'+'Internacional').then(function(portada){
	...
	/*GET PARA TOP NEWS*/
	$http.get('/popularSeccion/'+'Internacional').then(function(tops){
	...
	$scope.cargaMas=function(){
		pagina++;
		$http.get('cargaMas/'+pagina+'/'+4).then(function(n){
	...
	$scope.anoticia=function(alias){
		$state.go('noticia',{id:alias});
	};	
});